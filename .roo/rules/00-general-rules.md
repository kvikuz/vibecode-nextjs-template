...
- Собирайте приложение с помощью docker, всегда используйте существующий в проекте Dockerfile и только если его нет, создавайте с нуля
- Всегда проверяй, что приложение собирается без ошибок с помощью команды `npm run build` перед тем, как собирать docker-образ
- Разворачивайте всегда в Yandex Cloud с использованием сервиса Serverless Containers
- Для развертывания используйте всегда terraform, ищите в проекте terraform-рецепты и применяйте их. Например, terraform-рецепты могут находиться внутри проекта в каталоге под названием `terraform` или `deploy`.
- Если в проекте есть shell-скрипты, автоматизирующие развертывание, всегда используйте их. Например, такой скрипт может называться `deploy.sh`.
